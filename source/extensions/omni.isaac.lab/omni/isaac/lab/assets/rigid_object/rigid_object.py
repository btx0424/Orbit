...
class RigidObject(AssetBase):
    ...
    def _initialize_impl(self):
        ...
        def get_root_prim_path(prim_path):
            template_prim = sim_utils.find_first_matching_prim(prim_path)
            ...
            leaf = root_prim_path.split("/")[-1]
            return root_prim_path[: -len(leaf)], leaf
        
        if isinstance(self.cfg.prim_path, str):
            prim_paths = self.cfg.prim_path
        else:
            prim_paths = list(self.cfg.prim_path)

        prefix, leaves = get_root_prim_path(prim_paths[0])
        for prim_path in prim_paths[1:]:
            prefix_, leaf = get_root_prim_path(prim_path)
            if prefix_ != prefix:
                raise ValueError("Prim paths must have the same prefix.")
            leaves = leaves + "|" + leaf
        root_prim_path_expr = prefix + "(" + leaves + ")"
        # -- object view
